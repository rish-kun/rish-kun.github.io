"use client";

const skills = ["Backend", "DevOps", "ML"];

const socialLinks = [
  {
    label: "GitHub",
    href: "https://github.com/rish-kun",
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z" />
      </svg>
    ),
  },
  {
    label: "LinkedIn",
    href: "https://linkedin.com/in/rish-kun",
    icon: (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
  {
    label: "X / Twitter",
    href: "https://x.com/",
    icon: (
      <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.259 5.631 5.905-5.631zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
];

export default function Hero() {
  return (
    <div className="flex flex-col h-full pl-16 lg:pl-24 pr-4 py-10">
      <div className="flex-1 flex flex-col justify-center">
        <p className="text-xs uppercase tracking-[0.2em] text-neutral-400 mb-3 font-medium">
          Hello, I&apos;m
        </p>

        <h1 className="text-5xl lg:text-6xl font-bold tracking-tight text-neutral-900 leading-none">
          Rishit
        </h1>

        <p className="text-base text-neutral-500 mt-3 font-normal">
          CS Student @ BITS Pilani
        </p>

        <div className="flex flex-wrap gap-2 mt-5">
          {skills.map((skill) => (
            <span
              key={skill}
              className="text-xs font-medium text-accent bg-accent-light px-3 py-1.5 rounded-full"
            >
              {skill}
            </span>
          ))}
        </div>

        <p className="text-sm text-neutral-500 mt-5 leading-relaxed max-w-sm">
          Backend and DevOps at DVM. Building for Oasis & Apogee.
        </p>

        <div className="flex items-center gap-3 mt-7">
          {socialLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={link.label}
              className="w-9 h-9 rounded-full bg-neutral-900 text-white flex items-center justify-center hover:bg-neutral-700 transition-colors duration-200"
            >
              {link.icon}
            </a>
          ))}
        </div>
      </div>

      {/* Currently status */}
      <div className="flex flex-col gap-3 mt-6">
        <div className="w-full h-px bg-neutral-200" />
        <div className="flex items-start gap-3">
          <span className="mt-1 w-1.5 h-1.5 rounded-full bg-green-400 flex-shrink-0" />
          <div>
            <p className="text-xs uppercase tracking-[0.15em] text-neutral-400 font-medium mb-0.5">
              Currently
            </p>
            <p className="text-sm text-neutral-600 leading-relaxed">
              Exploring agentic frameworks and LLM fine-tuning at university.
              Open to internships.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}
